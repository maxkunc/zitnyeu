        ...
        {major.length > 0 && (
          <div className="mt-16">
            ...
            <div className="mt-6 grid lg:grid-cols-2 gap-5">
              {major.map((p) => (
                <article key={p.id} className="group relative glass rounded-2xl overflow-hidden hover:border-primary/50 transition-all">
            ...
            <div className="relative rounded-3xl overflow-hidden glass border-primary/40 shadow-glow p-8 md:p-12">
              ...
              <div className="relative">
                <div className="inline-flex items-center gap-2 rounded-full bg-gradient-cyber px-4 py-1.5 text-xs font-mono font-semibold text-primary-foreground">
                  ...
                  {t("projects_esa_badge")}
                </div>
                <h3 className="mt-6 font-display text-3xl md:text-4xl font-bold max-w-2xl">{t("projects_esa_title")}</h3>
                <p className="mt-3 text-muted-foreground max-w-2xl">{t("projects_esa_desc")}</p>

                <div className="mt-10 grid md:grid-cols-2 gap-5">
                  {esa.map((p) => (
                    <div key={p.id} className="relative rounded-2xl p-6 bg-background/50 border border-primary/30 hover:border-primary transition-colors">
        ...
        {minor.length > 0 && (
          <div className="mt-20">
            ...
            <div className="mt-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {minor.map((p) => (
                <div key={p.id} className="glass rounded-xl p-5 hover:border-primary/40 hover:translate-y-[-2px] transition-all">
                  <div className="flex items-center justify-between">
                    <h4 className="font-display font-semibold">{p.title}</h4>
                    <span className="text-[10px] font-mono uppercase tracking-wider px-2 py-1 rounded bg-primary/15 text-primary">{p.status}</span>
                  </div>
                  <p className="mt-2 text-sm text-muted-foreground">{p.description}</p>
                </div>
              ))}
