...
import { Rocket, Menu, X, Languages } from "lucide-react";
  ...
  return (
    <header className={`fixed top-0 inset-x-0 z-50 transition-all ${scrolled ? "glass shadow-card" : "bg-transparent"}`}>
      <div className="mx-auto max-w-7xl px-6 py-3 flex items-center justify-between gap-4">
        <a href="#domu" className="flex items-center gap-2 group shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="lg:hidden p-2 rounded-md hover:bg-secondary" onClick={() => setOpen((v) => !v)} aria-label="Menu">
        ...
        <div className="lg:hidden glass border-t border-border">
          <div className="px-6 py-3 flex flex-col">
            ...
            <div className="flex gap-2 pt-3">
              <button onClick={() => setLang("cs")} className={`px-3 py-1.5 rounded-md text-xs font-mono ${lang === "cs" ? "bg-primary text-primary-foreground" : "glass"}`}>CZ</button>
              <button onClick={() => setLang("en")} className={`px-3 py-1.5 rounded-md text-xs font-mono ${lang === "en" ? "bg-primary text-primary-foreground" : "glass"}`}>EN</button>
            </div>
          </div>
        </div>
      )}
